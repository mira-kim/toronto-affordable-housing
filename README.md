...
- **Highlights available buildings** on the map when TCHC units are posted
- **Email alerts** — subscribers are notified when new TCHC listings appear
...
| `map_toronto_housing.py` | Reads the geocoded CSV and generates `data/toronto_housing_map.html` |
...
| `tests/test_monitor_tchc.py` | Unit tests for the TCHC scraper parsing logic |
